...
namespace Launcher
{
    public partial class FormMain : Form
    {
        ...
        void listView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                ...
                catch
                {
                    ...
                    MessageBox.Show("Не удалось прочитать файл: " + listView1.SelectedItems[0].ToolTipText);
                }
                for (int i = 6; i >= 0; i--)
                {
        ...
        void button1_Click(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                ...
                List<string> cacheFile = new List<string>()
                {
                    "[Settings]",
                    "Name=" + textBox1.Text,
                    "Firestorm=" + (checkBox9.Checked ? "yes" : "no"),
                    "ShortGame=" + (checkBox4.Checked ? "yes" : "no"),
                    "MultiEngineer=" + (checkBox8.Checked ? "yes" : "no"),
                    "MCVRedeploy=" + (checkBox11.Checked ? "yes" : "no"),
                    "Crates=" + (checkBox3.Checked ? "yes" : "no"),
                    "Bases=" + (checkBox1.Checked ? "yes" : "no"),
                    "HarvesterTruce=" + (checkBox14.Checked ? "yes" : "no"),
                    "AlliesAllowed=" + (checkBox2.Checked ? "yes" : "no"),
                    "BridgeDestroy=" + (checkBox10.Checked ? "yes" : "no"),
                    "FogOfWar=" + (checkBox7.Checked ? "yes" : "no"),
                    "BuildOffAlly=" + (checkBox13.Checked ? "yes" : "no"),
                    "MultipleFactory=" + (checkBox5.Checked ? ".85" : "0"),
                    "AimableSams=" + (checkBox6.Checked ? "yes" : "no"),
                    "SharedControl=" + (checkBox15.Checked ? "yes" : "no"),
                    "AttackNeutralUnits=" + (checkBox12.Checked ? "yes" : "no"),
                    "Side=" + (comboBox1.SelectedIndex == 2 ? random.Next(2) : comboBox1.SelectedIndex).ToString(),
                    "Scenario=spawnmap.ini",
                    "GameSpeed=" + numericUpDown3.Value.ToString(),
                    "ReplayRecording=yes",
                    "Color=" + (comboBox2.SelectedIndex == 8 ? random.Next(8) : comboBox2.SelectedIndex).ToString(),
                    "Port=8054",
                    "Seed=1234567890",
                    "Credits=" + numericUpDown1.Value.ToString(),
                    "TechLevel=" + numericUpDown2.Value.ToString(),
                    "Host=yes",
                    "GameID=1234567890",
                    "UnitCount=" + numericUpDown4.Value.ToString()
                };
                int aicount = 0;
                ...
                {
                    MessageBox.Show("Не удалось записать: " + Path.Combine(path, "spawn.ini") + " или копировать файл: " + listView1.SelectedItems[0].ToolTipText + " в " + Path.Combine(path, "spawnmap.ini"));
                }
        ...
        void startGame(string exe, string arg = null)
        {
            ...
            {
                MessageBox.Show("Не удалось запустить: " + process.StartInfo.FileName);
            }
            Application.Exit();
        }
        ...
        string lineReadWrite(string file, string section, string key, bool range = false, bool players = false, bool write = false, string value = null)
        {
            ...
            {
                MessageBox.Show("Не удалось прочитать файл: " + file);
            }
            bool find = false;
            bool success = false;
            string line = "";
